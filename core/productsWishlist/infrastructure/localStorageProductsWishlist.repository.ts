...
import { ProductWishlist, ProductWithAddedAt } from "../domain/productWishlist";
...
export const localStorageProductsWishlist = (): ProductWishlistRepository => {
  ...
  return {
    ...
    async toggleProduct({ userId, product }) {
      ...
      const productWithAddedAt: ProductWithAddedAt = {
        ...product,
      ...
      if (!wishlist) {
        ...
        newWishlist.products.push(productWithAddedAt);
      ...
      if (productIndex > -1) {
        ...
        wishlist.products.push(productWithAddedAt);
